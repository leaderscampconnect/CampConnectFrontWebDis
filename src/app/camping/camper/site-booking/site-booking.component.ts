...
import { SiteAvailability } from '../../../models/camping-models';;
...
@Component({
  ...
  styleUrl: './site-booking.component.scss'
})
export class SiteBookingComponent implements OnInit {
  ...
  loadSelectedSite(siteId: number): void {
    ...
    this.campingService.getCampingSiteById(siteId).subscribe({
      next: (site) => {
        ...
        this.bookingForm.siteId = site.idSite || 0;
  ...
  checkAvailability(): void {
    ...
    this.campingService.getSiteAvailability(
      ...
      next: (data) => {
        ...
        const cap = data.remainingCapacity || 0;
        if (this.bookingForm.numberOfGuests > cap) {
          this.errorMessage = `Only ${cap} guest(s) available for these dates.`;
        }
      },
      error: (error) => {
        console.error(error);
  ...
  onSubmit(): void {
    ...
    const remCap = this.availability.remainingCapacity || 0;
    if (remCap <= 0) {
      this.showError('No remaining spots are available for these dates.');
      return;
    }

    if (this.bookingForm.numberOfGuests > remCap) {
      this.showError(`Max allowed for these dates is ${remCap} guest(s).`);
      return;
    }

    this.router.navigate(['/camper/booking-summary'], {
      state: {
        bookingData: {
          ...this.bookingForm,
