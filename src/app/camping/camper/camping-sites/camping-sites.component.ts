...
import { AuthService } from '../../../core/auth.service';import { CampingSite } from '../../../models/camping-models';
;
...
@Component({
  ...
  styleUrl: './camping-sites.component.scss'
})
export class CampingSitesComponent implements OnInit {
  ...
  constructor(
    ...
    this.adminRole = this.authService.roles()[0] || '';
  }
  ...
  loadCampingSites(): void {
    ...
    this.campingService.getAllCampingSites().subscribe({
      next: (data) => {
        this.sites = data.filter(site => site.statutDispo !== 'FERME').reverse();
        this.applyFilters();
